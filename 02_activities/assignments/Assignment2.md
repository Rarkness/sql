...
```
One architecture (Type 1) overwrites old data whenever a customer’s address changes, resulting in no historical record, while another architecture (Type 2) adds new records for each change, preserving a full timeline of all past addresses. See drawing files in assignment directory.
```
